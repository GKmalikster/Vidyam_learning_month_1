...
-- Canonical learner identity, separate from registrations (which stays a
-- per-session snapshot/audit row — see comment there). This table exists so
-- a returning learner can sign in once and see/join every program they've
-- registered for, plus one-click join any program added after they first
-- signed up, without re-submitting the full profile form each time.
CREATE TABLE IF NOT EXISTS learners (
  ...
  time_pref TEXT DEFAULT '',
  is_returning TEXT DEFAULT '',
  goal TEXT DEFAULT '',
  source TEXT DEFAULT '',
...
CREATE TABLE IF NOT EXISTS learner_accounts (
  id SERIAL PRIMARY KEY,
  learner_id INTEGER NOT NULL REFERENCES learners(id) UNIQUE,
  password_hash TEXT NOT NULL,
  created_at TIMESTAMPTZ DEFAULT NOW()
);
...
-- Backs the self-serve "forgot password" flow for both learner and trainer
-- accounts (account_type distinguishes which credential table account_id
-- refers to). Tokens are single-use and short-lived; only a hash is stored
-- so a leaked database row alone can't be used to reset a password.
...
ALTER TABLE registrations ADD COLUMN IF NOT EXISTS learner_id INTEGER REFERENCES learners(id);
ALTER TABLE courses ADD COLUMN IF NOT EXISTS image TEXT DEFAULT '';
ALTER TABLE courses ADD COLUMN IF NOT EXISTS preview_image TEXT DEFAULT '';
`;
