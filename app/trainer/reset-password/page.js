...
function ResetForm() {
  ...
  async function submit(e) {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/trainer-auth/reset", {
        method: "POST",
    ...
    <div className="form-card" style={{ maxWidth: 380, width: "100%" }}>
      ...
      {!token && (
        <p style={{ color: "#c0392b", textAlign: "center", fontSize: 13.5 }}>
          This link is missing its reset token. Please request a new link from the <Link href="/trainer/forgot-password" style={{ color: "var(--blue)", fontWeight: 600 }}>forgot password</Link> page.
        </p>
      )}
