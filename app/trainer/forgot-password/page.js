...
export default function TrainerForgotPassword() {
  ...
  async function submit(e) {
    ...
    setError("");
    try {
      await fetch("/api/trainer-auth/forgot", {
        method: "POST",
        ...
        {sent ? (
          <>
            ...
            <p style={{ textAlign: "center", fontSize: 13, marginTop: 18 }}>
              <Link href="/trainer/login" style={{ color: "var(--blue)", fontWeight: 600 }}>Back to sign in</Link>
            </p>
          </>
        ) : (
          <form onSubmit={submit}>
            <p className="hint" style={{ textAlign: "center", display: "block", marginBottom: 20 }}>Enter the email your trainer application is under, and we will send you a reset link.</p>
